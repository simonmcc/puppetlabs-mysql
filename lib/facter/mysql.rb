...
#
# mysql_version is failing on my platform (OEL), 
# so carry on with the fact collection
...
variables_output = %x[mysqladmin variables].split("\n")
# get the key/value out of the output & store them in variables
variables = Hash.new
variables_output.each { |line| 
  parsed = /^\|\s(\w+)\s+\|\s+(\w+)/.match(line)
  if !parsed.nil?
    key,value = parsed.captures
    variables[key] = value
  end
}

variables.each { |key, value|
  Facter.add("mysql_#{key}") do
    setcode do
      value
    end
  end
